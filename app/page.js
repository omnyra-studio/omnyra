...
function AppShell({ screen, setScreen, subScreen, setSubScreen, activeTool, setActiveTool, mode, setMode, onSearch, onNotif, showToast }) {
  ...
  return (
    <div style={{minHeight:"100vh",paddingBottom:100}}>
      {screen==="home"    && <Home onTool={setActiveTool} mode={mode} setMode={setMode} onSearch={onSearch} onNotif={onNotif} credits={credits} brand={brand} onBrandPanel={()=>setBrandPanelOpen(true)}/>}
...
/* ── HOME ── */
function Home({ onTool, mode, setMode, onSearch, onNotif, credits, brand, onBrandPanel }) {
...
function ScriptStudio({ mode, setMode, onBack, showToast, brand }) {
  ...
  const genDirections = async () => {
    ...
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"script",phase:"directions",prompt,mode,tone:tone?.id,platform:platform?.label,length:LENGTHS.find(l=>l.id===length)?.label,brand})});
      const data = await res.json();
  ...
  const expand = async (dir) => {
    ...
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"script",phase:"expand",prompt,mode,tone:tone?.id,platform:platform?.label,length:LENGTHS.find(l=>l.id===length)?.label,direction:dir,brand})});
      const data = await res.json();
      if (data.error){setError(data.error);setGen(false);return;}
  ...
  const regen = async (section) => {
    setRK(section);
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"script",phase:"regenerate",prompt,mode,tone:tone?.id,section,context:prompt,brand})});
      const data = await res.json();
...
function OneClickFlow({ mode, setMode, onBack, showToast, brand }) {
  ...
  const genDirections = async () => {
    ...
    setGen(true); setError(null); setDirs(null);
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"oneclick",phase:"directions",prompt,mode,tone:tone?.id,platform:platform?.label,style:style?.label,brand})});
      const data = await res.json();
      if (data.error){setError(data.error);setGen(false);return;}
  ...
  const expand = async (dir) => {
    setSel(dir.id-1); setGen(true); setExp(null);
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"oneclick",phase:"expand",prompt,mode,tone:tone?.id,platform:platform?.label,style:style?.label,direction:dir,brand})});
      const data = await res.json();
      if (data.error){setError(data.error);setGen(false);return;}
  ...
  const regen = async (section) => {
    setRK(section);
    try {
      const res  = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"oneclick",phase:"regenerate",prompt,mode,tone:tone?.id,platform:platform?.label,style:style?.label,section,context:prompt,brand})});
      const data = await res.json();
      if (data.result&&expanded){ const k=section.toLowerCase().replace(/\s+/g,''); setExp(p=>({...p,[k]:data.result.trim()})); showToast(`${section} regenerated! ✓`); }
    } catch { showToast("Regen failed"); }
    setRK(null);
...
function CaptionTool({ mode, setMode, onBack, showToast, brand }) {
  ...
  const generate = async () => {
    ...
    try {
      const res=await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:"caption",prompt,mode,brand})});
      const data=await res.json();
      if (data.error){setError(data.error);setGen(false);return;}
...
function GenericTool({ tool, mode, setMode, onBack, showToast, brand }) {
  ...
  const generate = async () => {
    ...
    if (isText) {
      try {
        const res=await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool:tool.id,prompt,mode,brand})});
        const data=await res.json();
        if (data.error){setError(data.error);setGen(false);return;}
...
function AccountScreen({ onBack, showToast }) {
  return (
    <div style={{padding:"56px 20px 40px"}}>
      ...
      <PressBtn onClick={()=>showToast("Signed out!")} style={{width:"100%",padding:"14px 18px",borderRadius:16,marginTop:8,background:"rgba(244,63,94,0.06)",border:"1px solid rgba(244,63,94,0.15)",display:"flex",alignItems:"center",gap:14,color:"#f43f5e",fontFamily:"inherit",textAlign:"left"}}>
