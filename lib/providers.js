...
export async function callSyncLabs({ videoUrl, audioUrl }) {
  console.error('[FATAL] SyncLabs execution path reached — callSyncLabs must never be called. SyncLabs has been replaced by Hedra. Investigate the call site immediately.', { videoUrl: videoUrl?.substring(0, 80), audioUrl: audioUrl?.substring(0, 80) })
  throw new Error('[FATAL] SyncLabs is decommissioned. Use Hedra via /api/avatar-worker.')
}

export async function pollSyncLabs(jobId) {
  console.error('[FATAL] SyncLabs execution path reached — pollSyncLabs must never be called. SyncLabs has been replaced by Hedra.', { jobId })
  throw new Error('[FATAL] SyncLabs is decommissioned. Use Hedra via /api/avatar-worker.')
}
