...
export async function refundCredits(userId, action, reason) {
  ...
  const db = getDb()
  const { data: credit } = await db.from('credits').select('balance').eq('user_id', userId).single()
  if (!credit) return

  const newBalance = (credit.balance ?? 0) + cost
  await db.from('credits').update({ balance: newBalance, updated_at: new Date().toISOString() }).eq('user_id', userId)
  await db.from('credit_transactions').insert({
    user_id: userId,
    amount: cost,
    type: 'refund',
