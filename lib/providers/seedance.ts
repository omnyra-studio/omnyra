...
/** Pass requested duration through unchanged (clamped 1–6s). */
...
/** Optimized Seedance Fast — i2v when image available, requested duration unchanged. */
export async function falSeedanceFastGenerate(
  ...
  const motionStrength = params.motionStrength ?? "high";
  ...
  console.log(`[SEEDANCE_FAST] Generating ${duration}s | i2v=${hasImage} | ${model}`);
...
export async function callSeedance(input: SeedanceGenerateInput): Promise<SeedanceGenerateResult> {
  const result = await falSeedanceFastGenerate({
    ...
    motionStrength: input.motionStrength ?? "high",
